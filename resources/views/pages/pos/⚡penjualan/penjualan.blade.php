<div style="--theme-color: {{ $themeColor }};" class="min-h-0 md:h-full flex flex-col p-2 sm:p-4 animate-fade-in text-zinc-800 dark:text-zinc-200">
    
    <div class="flex justify-between items-center mb-4 px-1">
        <h2 class="text-xl sm:text-2xl font-black text-[var(--theme-color)] flex items-center gap-2">
            <x-heroicon-o-shopping-cart class="w-6 h-6 sm:w-7 sm:h-7" /> POS Penjualan
        </h2>
        <div class="flex items-center gap-2">
            <button x-data="{ isDark: document.documentElement.classList.contains('dark') }" 
                ...
                class="p-2 bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 flex-1 overflow-visible lg:overflow-hidden pb-10 lg:pb-0">
        
        <div class="lg:col-span-2 flex flex-col gap-4 overflow-visible lg:overflow-hidden">
            <div class="relative shrink-0 px-1">
                ...
                <input wire:model.live.debounce.300ms="searchProduct" type="text" placeholder="Cari nama barang atau scan barcode..." 
                        class="w-full pl-11 pr-4 py-3 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 focus:ring-2 focus:ring-[var(--theme-color)] outline-none transition">
            </div>

            <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-2.5 overflow-y-visible lg:overflow-y-auto pb-4 px-1">
                @forelse($products as $product)
                    <button wire:click="addToCart({{ $product->id }})" class="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 p-3 rounded-xl flex flex-col items-center text-center hover:border-[var(--theme-color)] dark:hover:border-[var(--theme-color)] transition group shadow-sm">
                        <div class="w-12 h-12 sm:w-16 sm:h-16 bg-zinc-100 dark:bg-zinc-700 rounded-lg flex items-center justify-center mb-2 sm:mb-3 group-hover:scale-105 transition-transform">
                            <x-heroicon-o-cube class="w-6 h-6 sm:w-8 sm:h-8 text-zinc-400" />
                        </div>
                        <h3 class="text-xs sm:text-sm font-semibold line-clamp-2 text-zinc-700 dark:text-zinc-200">{{ $product->name }}</h3>
        ...
        <div class="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-2xl flex flex-col h-auto lg:h-full overflow-visible lg:overflow-hidden shadow-md mt-2 lg:mt-0 mx-1 lg:mx-0">
            
            <div class="p-4 border-b border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50 rounded-t-2xl shrink-0 flex flex-col gap-3">
                <div class="flex justify-between items-center">
                    <h2 class="font-bold flex items-center gap-2 text-sm sm:text-base">
                        <x-heroicon-o-shopping-cart class="w-5 h-5 text-[var(--theme-color)]" /> Keranjang Belanja
                    </h2>
                ...
                <select wire:model.live="customerId" class="w-full text-sm bg-white dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-600 text-zinc-800 dark:text-zinc-200 logic-customer rounded-lg px-3 py-2 focus:ring-2 focus:ring-[var(--theme-color)] outline-none transition font-semibold">
            ...
            <div class="flex-1 overflow-y-visible lg:overflow-y-auto p-3 sm:p-4 flex flex-col gap-3 min-h-0">
                @forelse($cart as $index => $item)
                    <div class="flex flex-col gap-3 p-3 border border-zinc-200 dark:border-zinc-700 rounded-xl bg-white dark:bg-zinc-800/80 shadow-sm relative group">
                        <div class="flex justify-between items-start pr-6">
                            <h4 class="text-sm font-bold line-clamp-2 text-zinc-800 dark:text-zinc-100 leading-snug">{{ $item['name'] }}</h4>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5 mt-1">
                            <div class="flex flex-col gap-2">
                                <div>
                                    <label class="text-[10px] Cesar text-zinc-500 font-bold uppercase tracking-wider mb-1 block">Satuan</label>
                                    @if(count($item['available_units']) > 0)
                                        <select wire:change="changeCartUnit({{ $index }}, $event.target.value)" class="w-full text-xs bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-md py-1.5 px-2 outline-none font-medium">
                            ...
                            <div class="flex flex-col gap-2">
                                <div>
                                    ...
                                    <div class="flex items-center gap-1">
                                        <button wire:click="decreaseQty({{ $index }})" class="w-7 h-7 flex items-center justify-center rounded-md bg-zinc-100 dark:bg-zinc-700 text-zinc-600 dark:text-zinc-400 hover:bg-red-100 transition"><x-heroicon-o-minus class="w-3 h-3" /></button>
                                        <input type="number" wire:model.live="cart.{{ $index }}.qty_billed" wire:change="validateCart({{ $index }})" class="w-full text-center font-bold text-sm bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-md py-1">
                        ...
                        <div class="mt-2 pt-2 border-t border-zinc-100 dark:border-zinc-700 flex justify-between items-center">
            ...
            <div class="p-4 border-t border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900/50 rounded-b-2xl shrink-0">
                <div class="flex justify-between items-center mb-4">
                    <span class="text-zinc-500 font-bold uppercase tracking-wider text-xs">Grand Total</span>
                    <span class="text-xl sm:text-2xl font-black text-[var(--theme-color)]">Rp {{ number_format($total, 0, ',', '.') }}</span>
                </div>
                <button wire:click="openCheckout" class="w-full bg-[var(--theme-color)] text-white font-black py-4 rounded-xl hover:opacity-90 transition shadow-lg shadow-blue-500/20 flex justify-center items-center gap-2 disabled:opacity-50 text-sm sm:text-base" {{ empty($cart) ? 'disabled' : '' }}>
                    <x-heroicon-o-check-circle class="w-5 h-5 sm:w-6 sm:h-6" /> Lanjut Pembayaran
                </button>
            </div>
        </div>
    </div>
